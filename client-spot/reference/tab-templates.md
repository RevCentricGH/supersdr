...
This tab feeds the Apollo Campaign Builder skill directly. Apollo keyword passes, employee ranges, tech signals, and exclusions all need to be specific enough that a list build can run off them without further input.
...
Domain: [example.com]
Campaign Contact: [Founder or operator name running outreach]

ICP FILTERS (for Apollo contact search)

Keyword Passes (Apollo will match these against company/contact descriptions):
- [keyword pass 1 — e.g. "AI voice agent"]
- [keyword pass 2 — e.g. "conversational AI"]
- [keyword pass 3 — e.g. "voice AI platform"]

Firmographics:
- Employee range: [min]–[max] (e.g. 11–50, 51–200, 201–500)
- Revenue range: [min]–[max]
- Locations: [countries/regions — e.g. United States, Canada]
- Industries: [list — match Apollo industry tags where possible]
- Funding stages: [e.g. Seed, Series A, Series B]

Target Titles (Primary — decision makers):
- [Title 1]
- [Title 2]
- [Title 3]

Target Titles (Secondary — influencers):
- [Title]
- [Title]

Tech Stack Signals (include — confirms ICP fit):
- [e.g. Salesforce, HubSpot, Outreach]

Tech Stack Signals (exclude — disqualifiers):
- [e.g. already using a direct competitor product]

Exclusions:
- Competitor companies: [list]
- Title keywords to exclude: [e.g. recruiter, intern, executive assistant, COO, CFO]
- Company types to exclude: [e.g. universities, VCs, government, non-profits]

MESSAGING BRIEF (for Apollo Campaign Builder — pulled from Tabs 3, 4, 6, 7)

Primary value prop: [one-line description of what the product does]
Core pain (symptomese): [from Tab 4.3 — visceral, specific, not Google-able]
Key differentiator: [from Tab 4.5 — the structural moat in one sentence]
Top social proof: [2–3 notable clients from Tab 3]
Measurable impact: [specific number or outcome from Tab 4.4]

Top objection to avoid surfacing in outreach:
[From Tab 7 — the most common early objection and what language triggers it. Format: "Objection: [X]. Avoid: [Y]. Frame instead around: [Z]"]
```
