...
Public skills for the SuperSDR community (RevCentric), installable in any agentic harness. Each dir under `skills/` is one skill.

## Structure
- A skill is a folder under `skills/` with a `SKILL.md`. Frontmatter `name:` MUST equal the folder name; `description:` must be non-empty (it's the trigger) and at most 1024 chars (Claude rejects the skill at upload past that). Both keys need a space after the colon or YAML drops them. Optional `reference/` holds supporting docs.
