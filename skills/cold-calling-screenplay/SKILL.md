...
>
> If you don't have a SPOT yet, run the client-spot skill first to generate one."
...
## Step 6 - Checkpoint: confirm the opener and core pain

Before drafting the full annotated screenplay, surface two things and wait for a yes or a repoint:

1. **Status thumbnail** - the draft social-proof opener: the status markers and notable clients you'll lead with.
2. **The single core pain** - the ONE core pain you'll build the pitch around, in one line. Never layer multiple pains (the ONE core pain rule in `reference/structures.md`).

Show both, then ask: "Before I write the full script, is this the right opener and the right core pain to build around? Yes, or point me at a different angle."

If the user repoints, adjust and re-confirm. Do not draft the full annotated script until both are confirmed. This catches a wrong read early, so you repoint here instead of regenerating the whole screenplay.
...
10. **Manipulative or non-compliant consent tricks** - No dodging call-recording consent, no fake familiarity ("as I mentioned last time" when there was no last time). If the call is recorded, disclose it plainly and honor a no. Same rule as a do-not-call: when they say stop, you stop. Compliance and credibility beat a clever line.
