...
Report "TAM search saved" only after both STEP K save-verification checks pass: the view-name dropdown at the top of the results table shows the new search name (not "Default view"), AND the search name appears in the Saved Searches tab. If either check fails, do not print this report. Tell the user the save did not confirm, describe what you saw, and that they can save manually via the "Save as new search" button.

Once both checks pass, report back:
