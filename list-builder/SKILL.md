...
> **Claude Cowork skill. Runs in the Claude app, not a terminal.** No API keys, no scripts, no local files. Everything runs through MCP connectors.
...
### Stage 3 - ICP qualification on a sample (the checkpoint gate)

Qualify a small sample first, confirm the ICP read with the operator, then scale. This one checkpoint sits between qualification and the costly stages (Stage 4 reveal and Stage 6 Layer 4 enrichment), so a wrong ICP read does not waste enrichment spend across the whole batch.

**1. Qualify the sample.** Take the first ~10-15 Tier 1/2 contacts, ranked by Fit Score. For each unique company in the sample, do a quick check against the SPOT's ICP description and exclusions. Drop:
...
**2. Show the kept/dropped split with reasons.** Present the sample result so the operator can read the ICP before any spend:

> Sample qualified: [kept] of [sample size] kept.
>
> Kept:
>   - [Name], [Title] @ [Company] - Fit [score] - [one-line why it fits]
>   - ...
>
> Dropped:
>   - [Company] - [reason, e.g. "LLM gateway competitor", "research lab", "matches SPOT exclusion: X"]
>   - ...
>
> Does this read right? Reply "go" to lock these criteria and scale to the full list, or tell me what to tune: loosen or tighten an exclusion, adjust a scoring weight, or move a tier cutoff.

**3. Wait for confirm or tune.** Do not start Stage 4 reveal or Stage 6 enrichment until the operator responds.
- "go" / "confirm" -> lock the criteria as they stand.
- Any tuning instruction -> apply it (edit the exclusion list, the Stage 2 scoring weights, or the tier cutoffs), re-qualify the same sample, and show the updated split. Repeat until the operator confirms.

**4. Lock and scale.** Once confirmed, the locked criteria (exclusion list + scoring weights + tier cutoffs) are fixed for the rest of this run. Apply them to qualify the full Tier 1/2 list the same way, with no further prompts. Every stage below operates only on contacts that pass the locked criteria.
...
For each Tier 1 contact, do signal research and apply compound intent scoring. This is the step that produces the most dial value, so it runs only after the sample checkpoint locks the criteria.
