...
- The Conversation Math table, each per-tier label-value block (Duration / Investment / Projected Meetings / Activated Leads / What's Included), and the Investment Summary table each become a `table` block with `header` + `rows`. The Appendix A billable and non-billable disposition tables become `status_table` blocks (`header`, `rows`, and `status_col` set to the 0-based integer index of the Status column, e.g. `1` when the columns are Disposition / Status / Definition) so Billable renders green and Not Billable renders red. `status_col` must be an integer, not the column name; a string never matches and silently leaves the table uncolored.
- The Next Steps + Acceptance signature block becomes a `signature` block whose `parties` array holds one object per signer, each with `title`, `org`, `signer`, and `role` (the top-level key must be `parties`; any other key renders a blank signature block).
