...
description:This document outlines a specialized skill for building send-ready outbound agency proposals (Calling, Cold Email, or Multi-Channel) in Google Docs. Grounded in discovery-call summaries or transcripts, the skill targets sales contexts involving Done-For-You (DFY) outbound services. Key objectives include: (1) Integrating agency Terms & Conditions and Appendix A (Completed Conversation Criteria); (2) Conducting prospect web research; (3) Generating a complete proposal with executive summaries, pricing tiers, and signature blocks; and (4) Drafting follow-up emails for proposal delivery. The process is designed to handle clarifying questions and ensure the final document is ready for prospect review. Triggering occurs automatically when proposals are mentioned or call transcripts are provided. The model ensures results are professional, comprehensive, and tailored to the outbound sales service model.
