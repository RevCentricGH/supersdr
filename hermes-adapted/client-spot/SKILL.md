---
name: client-spot
description: Generate a client single point of truth (SPOT) document - multi-section Google Doc covering campaign status, company overview, problem/solution, ICP, competitive landscape, objections, screenplay, and Apollo campaign setup. Pulls from onboarding call transcripts, meeting summaries, Google form responses, and web research. Use when user says "create SPOT doc for [client]", "build the client KB for [client]", "set up SPOT for [client]", "generate client brief for [client]", "onboard [client]", or pastes a client onboarding transcript / meeting notes and asks to turn it into a SPOT.
author: RevCentric
tags: [revcentric, client-onboarding, spoot]
---

# Client SPOT Skill

## Autonomous replacement (Hermes Mode)

Run in Hermes. Replace all Cowork Google Drive connector calls with the built-in `google-workspace` skill (loaded with `/skill google-workspace`). Use `google_docs_create` to make the doc, then `google_docs_update` or the batch-write tool to insert sections. Formatting: use heading styles for the 9 sections, bold + list formatting for fields. Write in one pass (create → populate → verify). If the skill's Drive/Sheets tooling is unavailable, write all 9 sections as raw text in chat with a note to paste into a new Doc.

## Purpose

Generates a complete single point of truth document for a new client, structured as 9 sections in a Google Doc.
...
Generate all 9 sections in a single response. Output each as its own labeled block. Mark anything still unknown as `[TBD]`. Do not pause to confirm.
...
Load `reference/tab-templates.md` for the field-by-field templates for all 9 sections when generating content.

### Step 5 - Create the Google Doc

Use the built-in `google-workspace` skill to create the Doc. Write all 9 sections in order as heading-level items. Do not construct raw Drive API calls directly - load the skill and use its tools.

1. Load `/skill google-workspace` and confirm Docs tools are available.
2. Create a new Google Doc titled "[Client Name] Single Point of Truth".
3. Write 9 section headings in order: Campaign Status, Campaign Brief, Company Overview, Problem Solution, ICP & Buyer Persona, Competitor Overview, Objection Handling, Screenplay, Apollo Campaign Setup.
4. Populate each section's content under its heading.

**If Google Docs tooling is unavailable:** write all 9 section blocks as labeled text in chat with this note:
```
Google Docs unavailable: paste each section below into a new Doc, using the section names as heading-level separators.
```
