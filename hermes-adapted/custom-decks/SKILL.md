...
2. **Create your config.** Copy the template and fill it in. Your real config is gitignored.
   ...
   > **PITFALL:** The bundled `config.template.json` may contain stale or wrong schema (e.g., metrics tracking fields from another tool). Always verify the file matches the custom-decks schema below before running. If it doesn't, use the verified template at `templates/config.json` instead — copy that to `config.json`.
