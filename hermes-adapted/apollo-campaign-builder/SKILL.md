...
Assume Apollo is open in Chrome and browser automation is enabled. Use Hermes `browser_navigate` to go to `https://app.apollo.io/#/sequences` and start the workflow.
...
- **Browser navigation fails entirely / browser automation not available** → "Headless browser control isn't enabled. Go to Settings → Computer Use, turn on browser control, then come back."
...
After verification, output a Mermaid diagram showing how the 4 workflows route SDR dispositions into sequences. Hermes renders this natively — gives the user a single visual that maps the entire campaign logic.
