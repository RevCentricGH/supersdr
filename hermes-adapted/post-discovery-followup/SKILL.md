...
The whole path runs in one Hermes session. The transcript, the company name, the confirmed
outcome, the returned draft, and the recipient carry forward from step to step with no manual
stitching. The skill uses Hermes-native tools: google-workspace for Docs, himalaya for Gmail,
and `execute_code` against the `revcentric-tools/apollo_client.py` helper for all Apollo writes.
Every external action (the email send and the Apollo write) waits for the operator's explicit
approval.
...
Autonomous replacement (run this in Hermes):
- **Google Doc / proposal creation**: load `/skill google-workspace` and use `google_docs_create` + `google_docs_update` instead of the Cowork Google Drive connector.
- **Gmail send**: load `/skill himalaya` and use `himalaya send --to --subject --body` instead of the Cowork Gmail connector.
- **Apollo writes**: call `execute_code` against the client at `~/.hermes/profiles/revcentric/skills/revcentric-tools/apollo_client.py` instead of Claude-in-Chrome browser automation. Available methods cover all steps in Step 6: `search_opportunities()`, `update_opportunity_stage()`.
...
Use `execute_code` to invoke the Hermes `himalaya` email tool. Gmail is sent through himalaya,
not a Cowork connector. Load `/skill himalaya` if credentials are not yet configured. For the
Apollo steps, load `/skill revcentric-tools` and call `apollo_client.py` methods.
...
Look up the target stage for the confirmed outcome in `reference/outcome-taxonomy.md` (the
outcome-to-Apollo-stage map). The target stage is a logical label - the operator's Apollo
pipeline may name it differently or not have it - so confirm the real column with the operator
before any write.

Use `execute_code` to call `revcentric-tools/apollo_client.py`. Load `/skill revcentric-tools`
if needed, then run:
...
import sys
sys.path.append('/Users/kevintran/.hermes/profiles/revcentric/skills/revcentric-tools')
from apollo_client import ApolloClient
client = ApolloClient()

opportunities = client.search_opportunities(company_name="{company_name}")
for opp in opportunities:
    print(f"{opp['id']} | {opp.get('name', 'N/A')} | Stage: {opp.get('stageName', 'N/A')}")

client.update_opportunity_stage(opportunity_id="{OPPORTUNITY_ID}", stage_name="{TARGET_STAGE}")
""")
print(result['output'])
```

- Show the matched opportunity(s) and their current stages, then require an explicit operator "yes"
  before any write.
- On zero or multiple matches, let the operator pick or supply the correct opportunity ID. The calendar or
  transcript name often differs from the Apollo account name (agency vs end-client, campaign
  alias); the operator resolves that here. There is no alias table.
