...
>
> Autonomous mode: this skill runs in Hermes and uses the built-in `google-workspace` skill for Doc writes. If Docs tooling is unavailable, the brief is returned as formatted chat text for manual paste.

Assume the `google-workspace` skill is available. Proceed once the user provides a transcript or a link.

**Only if creating the Doc fails:** "Google Docs write failed. I'll hand you the brief as text to paste into a Doc yourself. Say the word and I'll start."
...
Use the built-in `google-workspace` skill to create and populate the brief. The skill provides native Docs tools; use them in place of the Cowork Google Drive connector.

1. Load `/skill google-workspace` and confirm Docs tools are available.
2. Create a Doc titled using the active template's title pattern: `Pre-Brief (Discovery): {prospect or meeting name}` for `pre-disco`, `Pre-Brief (Onboarding): {client or account name}` for `pre-onboarding`, `Pre-Brief (Closing): {prospect or account name}` for `pre-closing`.
3. Write the point section(s) using heading and body formatting, then the "Transcript anchors" section.
4. Capture the Doc URL once creation is confirmed. Emit `[pre-brief] event=doc-created detail=<active type>`.
5. If creation fails, emit `[pre-brief] event=doc-failed detail=<reason>` and fall back to text output as described in the Skill intro.

The templates above are content specs, not literal text. Apply bold through the tool's formatting, never literal `**` characters in the Doc.
