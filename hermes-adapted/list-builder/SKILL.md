---
name: list-builder
description: Build an enriched, dial-ready contact list from a client's SPOT doc using Apollo MCP and Hermes tools. Pure Hermes — no API keys needed at runtime, uses registered MCP servers. Outputs a Google Sheet (or inline table) ready for downstream channel tools. Use when user says "build me a list for [client]", "dial list for [client]", "build the list", "run list builder", "build contacts for [client]", "pull Apollo contacts for [client]", or "enrich my contact list".
---
...
Required MCPs the user must have connected to their Hermes account:
...
> "Apollo MCP isn't connected. Add it in Hermes → Settings → MCP Servers → Apollo. Then ask me again."
...
- **TAM mode**: pull contacts from the saved label / list ID via `apollo_perceive` or direct MCP call
...
For contacts with locked emails, call Apollo MCP's enrich/match tool to reveal email + phone. Use `apollo_perceive` or the direct MCP enrichment endpoint. Apollo returns its own email status (verified/unverified) — keep it as a first-pass signal, but the next stage does the real validation if the right MCPs are connected.
...
**Use parallel sub-agents.** Layer 4 is research-heavy — sequential web search across 30+ unique companies is slow and burns context. Spawn parallel sub-agents using Hermes `delegate_task` instead:
...
- Spawn one sub-agent per batch using `delegate_task` with toolsets `['web', 'search']`
...
- Run all sub-agents in parallel (single message, multiple delegate_task calls)
- Synthesize results when they return
- If delegation isn't available in this runtime, run the batches as sequential web searches — slower, same output
...
If Perplexity MCP / Apollo MCP / other research tools are connected, sub-agents use them automatically — they inherit the user's Hermes MCP environment.
...
Include a Mermaid funnel diagram showing how contacts moved through each stage. Hermes renders this natively — the user sees the drop-off visually instead of just reading numbers.
